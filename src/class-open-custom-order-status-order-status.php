 ...
 * @since 1.0.0
 */
class Open_Custom_Order_Status_Order_Status {
	...
	public function get_type() {
		...
		$type = __( 'Custom', 'open-custom-order-status' );
		...
		if ( in_array( $this->get_slug( true ), $wc_core_statuses, true ) ) {
			$type = __( 'Core', 'open-custom-order-status' );
		} else if ( $this->is_core_status() ) {
			$type = __( 'Third Party', 'open-custom-order-status' );
		}
	...
	public function is_core_status() {
		...
		$core_statuses = open_custom_order_status()->get_order_statuses_instance()->get_core_order_statuses();
