...
if ( ! class_exists( 'Open_Custom_Order_Status_Order_Status_Email' ) ) :

/**
 * Open Custom Order Status Order Status Email
 *
 * A generic email class for custom order status emails.
 *
 * @since 1.0.0
 */
class Open_Custom_Order_Status_Order_Status_Email extends WC_Email {
	...
	public function __construct( $id, array $args ) {
		...
		switch ( $this->type ) {
			...
			case 'customer':
				...
				$this->heading           = __( 'Order has been updated', 'open-custom-order-status' );
				$this->subject           = __( 'Regarding your {site_title} order from {order_date}', 'open-custom-order-status' );
				$this->default_body_text = __( 'Your order is now {order_status}. Order details are shown below for your reference:', 'open-custom-order-status' );

				if ( ! $this->recipient ) {
					$this->recipient = __( 'Customer', 'open-custom-order-status' );
				}
			...
			case 'admin':
				...
				$this->heading           = __( 'Order has been updated', 'open-custom-order-status' );
				$this->subject           = __( '[{site_title}] Customer order ({order_number}) updated', 'open-custom-order-status' );
				$this->default_body_text = __( 'This order is now {order_status}. Order details are as follows:', 'open-custom-order-status' );
		...
		if ( ! empty( $_GET['delete_template'] ) && ( $template = esc_attr( basename( wc_clean( $_GET['delete_template'] ) ) ) ) && ! empty( $this->$template ) ) {
			add_action( 'woocommerce_delete_email_template', array( $this, 'reset_templates' ), 10, 2 );
		}
	...
	public function init_form_fields() {
		...
		$form_fields = array(
			...
			'enabled' => array(
				'title'       => __( 'Enable/Disable', 'open-custom-order-status' ),
				'type'        => 'checkbox',
				'label'       => __( 'Enable this email notification', 'open-custom-order-status' ),
				'default'     => 'no',
			),

			'recipient' => array(
				'title'       => __( 'Recipient(s)', 'open-custom-order-status' ),
				'type'        => 'text',
				/* translators: %s - admin email as default option */
				'description' => sprintf( __( 'Enter recipients (comma separated) for this email. Defaults to <code>%s</code>.', 'open-custom-order-status' ), esc_attr( get_option( 'admin_email' ) ) ),
				'placeholder' => '',
				'default'     => '',
			),

			'subject' => array(
				'title'       => __( 'Subject', 'open-custom-order-status' ),
				'type'        => 'text',
				/* translators: %s - the default subject text */
				'description' => sprintf( __( 'This controls the email subject line. Leave blank to use the default subject: <code>%s</code>.', 'open-custom-order-status' ), $this->subject ),
				'placeholder' => '',
				'default'     => '',
			),

			'heading' => array(
				'title'       => __( 'Email Heading', 'open-custom-order-status' ),
				'type'        => 'text',
				/* translators: %s - the default heading text */
				'description' => sprintf( __( 'This controls the main heading contained within the email notification. Leave blank to use the default heading: <code>%s</code>.', 'open-custom-order-status' ), $this->heading ),
				'placeholder' => '',
				'default'     => '',
			),

			'body_text' => array(
				'title'       => __( 'Email Body', 'open-custom-order-status' ),
				'type'        => 'textarea',
				'description' => __( 'Optional email body text. You can use the following placeholders: <code>{order_date}, {order_number}, {order_status}, {billing_first_name}, {billing_last_name}, {billing_company}, {blogname}, {site_title}</code>', 'open-custom-order-status' ),
				'placeholder' => '',
				'default'     => $this->default_body_text,
			),

			'email_type' => array(
				'title'       => __( 'Email type', 'open-custom-order-status' ),
				'type'        => 'select',
				'description' => __( 'Choose which format of email to send.', 'open-custom-order-status' ),
				'default'     => 'html',
				'class'       => 'email_type',
				'options'     => array(
					'plain'       => __( 'Plain text', 'open-custom-order-status' ),
					'html'        => __( 'HTML', 'open-custom-order-status' ),
					'multipart'   => __( 'Multipart', 'open-custom-order-status' ),
				),
			),
	...
	public function trigger( $order_id ) {
		...
		if ( $order ) {
			...
			$status = new Open_Custom_Order_Status_Order_Status( $order_id );
			...
			$this->placeholders['{order_date}']         = $this->object->get_date_created() ? date_i18n( wc_date_format(), $this->object->get_date_created()->getTimestamp() ) : '';
			$this->placeholders['{order_number}']       = $this->object->get_order_number();
			$this->placeholders['{order_status}']       = wc_get_order_status_name( $this->object->get_status() );
			$this->placeholders['{billing_first_name}'] = $this->object->get_billing_first_name();
			$this->placeholders['{billing_last_name}']  = $this->object->get_billing_last_name();
			$this->placeholders['{billing_company}']    = $this->object->get_billing_company();

			$this->handleDeprecatedFilters();

			/**
			 * Filter the placeholders in subject, heading and body text.
			 *
			 * @since 1.15.6
			 *
			 ...
			 */
			$this->placeholders = apply_filters( 'wc_order_status_manager_order_status_email_placeholders', $this->placeholders, $this->id, $this->type, $this->object );
		}
		 ...
		 *
		 * Builds and filters the old $find and $replace arrays from the new $placeholders
		 * to support plugins that might still be using the old filter hooks.
		 *
		 * @since 1.15.6
		 */
		protected function handleDeprecatedFilters()
		{
			if (! $this->shouldHandleDeprecatedFilters()) {
				return;
			}

			// Build and filter the old $find array
			$find = [];
			foreach ($this->placeholders as $key => $value) {
				$find[str_replace('_', '-', str_replace(['{', '}'], '', $key))] = $key;
			}
			$find = $this->getFilteredFindVariables($find);

			// Build and filter the old $replace array
			$replace = [];
			foreach ($find as $key => $placeholder) {
				if (isset($this->placeholders[$placeholder])) {
					$replace[$key] = $this->placeholders[$placeholder];
				}
			}
			$replace = $this->getFilteredReplaceVariables($replace);

			// Update placeholders based on filtered $find and $replace
			foreach ($find as $key => $placeholder) {
				if (isset($replace[$key])) {
					$this->placeholders[$placeholder] = $replace[$key];
				}
			}
		}

		protected function getFilteredFindVariables(array $find): array
		{
			/**
			 ...
			 * @param \WC_Order $order the order
			 */
			return (array) apply_filters_deprecated('wc_order_status_manager_order_status_email_find_variables', [$find, $this->id, $this->type, $this->object], '1.15.6', 'wc_order_status_manager_order_status_email_placeholders');
		}

		protected function getFilteredReplaceVariables(array $replace): array
		{
			/**
			 ...
			 * @param \WC_Order $order the order
			 */
			return (array) apply_filters_deprecated('wc_order_status_manager_order_status_email_replace_variables', [$replace, $this->id, $this->type, $this->object], '1.15.6', 'wc_order_status_manager_order_status_email_placeholders');
		}

		protected function shouldHandleDeprecatedFilters(): bool
		{
			return has_filter('wc_order_status_manager_order_status_email_find_variables') || has_filter('wc_order_status_manager_order_status_email_replace_variables');
		}
	...
	public function get_body_text() {
		 ...
		 * @param \Open_Custom_Order_Status_Order_Status_Email $email the email object
		 */
