...
/**
 * Open Custom Order Status Order Statuses class
 *
 * @since 1.0.0
 */
class Open_Custom_Order_Status_Order_Statuses {
	...
	public function __construct() {
		...
		require_once( open_custom_order_status()->get_plugin_path() . '/src/class-open-custom-order-status-order-status.php' );
	...
	/**
	 * Gets the core order statuses that require payment. These are the statuses that cannot have their "Needs Payment"
	 * field edited to avoid interfering with processing errors.
	 *
	 * @since 1.15.5
	 * @return string[]
	 */
	public function getCoreOrderStatusesThatRequirePayment() : array
	{
		return ['pending', 'checkout-draft'];
	}
	...
	public function order_paid_statuses() {
		...
		foreach ( $custom_statuses as $order_status_post ) {
			...
			$custom_status = new Open_Custom_Order_Status_Order_Status( $order_status_post );
	 ...
	 * @return array
	 */
	public function order_needs_payment_statuses() : array {

		$custom_needs_payment_statuses = $this->getCoreOrderStatusesThatRequirePayment();
		$custom_statuses               = $this->get_order_status_posts();
		...
		return array_values(array_unique($custom_needs_payment_statuses));
	}
	...
	public function order_statuses_valid_for_payment_complete( $valid_order_statuses = array() ) {
		...
		foreach ( $custom_statuses as $custom_status_post ) {
			...
			$custom_status = new Open_Custom_Order_Status_Order_Status( $custom_status_post );
	...
	public function queue_orders_update( $post_id, $data ) {
		...
		$order_status = new Open_Custom_Order_Status_Order_Status( $post_id );
	...
	public function handle_order_status_delete( $post_id, $replacement_status = 'on-hold' ) {
		...
		$new_status = new Open_Custom_Order_Status_Order_Status( $replacement_status );
		...
		if ( ! empty( $order_rows ) ) {
			...
			foreach ( $order_rows as $order_row ) {
				...
				$order->update_status( $replacement_status, sprintf( __( 'Order status updated from %1$s to %2$s because the former status was deleted.', 'open-custom-order-status' ), $old_status_name, $new_status_name ) );
		...
		if ( $num_updated && is_admin() && ! wp_doing_ajax() ) {
			...
			$message = sprintf( _n(
				...
				$num_updated,
				'open-custom-order-status'
			), $num_updated, esc_html( $old_status_name ), esc_html( $new_status_name ) );

			open_custom_order_status()->get_message_handler()->add_message( $message );
		}
	}
	...
	/**
	 * Remove Open Custom Order Status actions from Order actions
	 *
	 * @see Open_Custom_Order_Status_Admin_Orders::custom_order_actions()
	 *
	 * @since 1.4.3
	 *
	...
	public function trim_order_actions( $order_actions ) {
		...
		if ( $order_statuses = $this->get_order_status_posts() ) {
			...
			foreach ( $order_statuses as $post ) {
				...
				if ( $status = new Open_Custom_Order_Status_Order_Status( $post ) ) {
	...
	public function get_custom_order_actions( $order ) {
		...
		$status         = new Open_Custom_Order_Status_Order_Status( $order->get_status() );
