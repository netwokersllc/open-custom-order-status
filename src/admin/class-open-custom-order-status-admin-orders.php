...
/**
 * Open Custom Order Status Orders Admin
 *
 * @since 1.0.0
 */
class Open_Custom_Order_Status_Admin_Orders {
	...
	public function custom_order_actions( $actions, WC_Order $order ) {
		...
		$custom_actions = $custom_actions = open_custom_order_status()->get_order_statuses_instance()->get_custom_order_actions( $order );

		if ( ! empty( $custom_actions ) ) {
			$actions = array_merge( $custom_actions, open_custom_order_status()->get_order_statuses_instance()->trim_order_actions( $actions ) );
		}
	...
	protected function get_custom_emails() {
		...
		if ( empty( $this->custom_emails ) ) {
			$this->custom_emails = open_custom_order_status()->get_emails_instance()->get_emails();
		}
	...
	public function custom_email_order_actions( $actions ) {
		...
		if ( ! empty( $custom_emails ) ) {
			...
			foreach ( $custom_emails as $email ) {
				...
				$new_actions["send_osm_email_{$email->post_name}"] = sprintf( __( 'Send %s custom email', 'open-custom-order-status' ), $email->post_title );
			}
	...
	public function send_custom_order_email( $order ) {
		...
		if ( ! empty( $_POST['wc_order_action'] ) ) {
			...
			if ( Framework\SV_WC_Helper::str_starts_with( $action, 'send_osm_email_' ) ) {
				...
				if ( $email instanceof \Open_Custom_Order_Status_Order_Status_Email ) {

					$email->trigger( $order->get_id(), $order );

					/* translators: Placeholders: %s - custom email name */
					$success = sprintf( __( '%s email notification manually sent.', 'open-custom-order-status' ), $email_post->post_title );

					$order->add_order_note( $success, false, true );
					open_custom_order_status()->get_message_handler()->add_message( $success );

				} else {

					/* translators: Placeholders: %s - custom email name */
					$message = sprintf( __( "%s email notification couldn't be sent.", 'open-custom-order-status' ), $email_post->post_title );

					open_custom_order_status()->get_message_handler()->add_error( $message );
				}
	...
	public function custom_order_status_icons() {
		...
		foreach ( wc_get_order_statuses() as $slug => $name ) {
			...
			// Font icon
			if ( $icon && $icon_details = open_custom_order_status()->get_icons_instance()->get_icon_details( $icon ) ) {
			...
			// Font action icon
			if ( $action_icon && $action_icon_details = open_custom_order_status()->get_icons_instance()->get_icon_details( $action_icon ) ) {
				...
				<?php foreach ( $custom_status_colors as $status => $color ) : ?>
					...
					<?php if ( in_array( $status, $custom_status_badges, true ) ) : ?>
						...
						<?php echo esc_html( $selector . $status ); ?> {
							...
							color: <?php echo esc_html( open_custom_order_status()->get_icons_instance()->get_contrast_text_color( $color ) ); ?>;
						}
					<?php endif; ?>
	...
	public function add_bulk_actions( array $actions ) {
		...
		$custom_order_statuses = open_custom_order_status()->get_order_statuses_instance()->get_order_status_posts( [
		...
		foreach ( $custom_order_statuses as $custom_order_status ) {
			...
			$status = new \Open_Custom_Order_Status_Order_Status( $custom_order_status );

			/* translators: Placeholder: %s - order status name */
			$label  = sprintf( __( 'Mark %s', 'open-custom-order-status' ), esc_html( strtolower( $status->get_name() ) ) );
