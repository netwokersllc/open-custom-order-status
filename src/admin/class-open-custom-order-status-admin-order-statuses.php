...
/**
 * Open Custom Order Status Order Statuses Admin
 *
 * @since 1.0.0
 */
class Open_Custom_Order_Status_Admin_Order_Statuses {
	...
	public function order_status_columns( $columns ) {
		...
		$columns['slug']        = __( 'Slug', 'open-custom-order-status' );
		$columns['description'] = __( 'Description', 'open-custom-order-status' );
		$columns['paid']        = __( 'Paid', 'open-custom-order-status' );
		$columns['reports']     = __( 'Reports', 'open-custom-order-status' );
		$columns['type']        = __( 'Type', 'open-custom-order-status' );

		$first_column = array( 'icon' => __( 'Icon', 'open-custom-order-status' ) );
	...
	public function order_status_actions( $actions, WP_Post $post ) {
		...
		$status = new Open_Custom_Order_Status_Order_Status( $post->ID );
	...
	public function add_meta_boxes() {
		...
		add_meta_box(
			...
			__( 'Order Status Data', 'open-custom-order-status' ),
		...
		add_meta_box(
			...
			__( 'Order Status Actions', 'open-custom-order-status' ),
		...
		<div id="order_status_options" class="panel woocommerce_options_panel">
			<div class="options_group">
				...
				woocommerce_wp_text_input( array(
					...
					'desc_tip'          => true,
					'description'       => __( 'Maximum 35 characters.', 'open-custom-order-status' ),
				) );
				...
				woocommerce_wp_text_input( array(
					'id'                => 'post_name',
					'label'             => __( 'Slug', 'open-custom-order-status' ),
					'value'             => $post->post_name,
					'custom_attributes' => $slug_custom_attributes,
					'desc_tip'          => true,
					'description'       => __( 'Optional. If left blank, the slug will be automatically generated from the name. Maximum: 17 characters, cannot contain leading numbers.', 'open-custom-order-status' ),
				) );
				...
				woocommerce_wp_textarea_input( array(
					...
					'desc_tip'    => true,
					'description' => __( 'Optional status description. If set, this will be shown to customers while viewing an order.', 'open-custom-order-status' ),
			...
			<div class="options_group">
				...
				woocommerce_wp_text_input( array(
					'id'          => '_color',
					'label'       => __( 'Color', 'open-custom-order-status' ),
					'type'        => 'text',
					'class'       => 'colorpick',
					'default'     => '#000000',
					'description' => __( 'Color displayed behind the order status image or name', 'open-custom-order-status' ),
				) );
				...
				<p class="form-field _icon_field">
					<label for="_icon"><?php esc_html_e( 'Icon', 'open-custom-order-status' ); ?></label>
					...
					<a href="#_icon" class="button button-small upload-icon upload-icon-image" data-uploader-button-text="<?php esc_html_e( 'Set as status icon', 'open-custom-order-status' ); ?>"><?php esc_html_e( "Select File", 'open-custom-order-status' ); ?></a>
					<a href="#_icon" class="button button-small remove-icon" ><?php esc_html_e( "Remove Icon", 'open-custom-order-status' ); ?></a>
					<?php echo wc_help_tip( __( 'Optional status icon. If not supplied, then Name will be displayed to represent the status', 'open-custom-order-status' ) ); ?>
				</p>
				...
				<p class="form-field _action_icon_field">
					<label for="_action_icon"><?php esc_html_e( 'Action Icon', 'open-custom-order-status' ); ?></label>

					<input
						type="hidden"
						id="_action_icon"
						name="_action_icon"
						value="<?php echo esc_attr( $action_icon ); ?>"
						data-icon-image="<?php echo esc_attr( $action_icon_attachment_src ? $action_icon_attachment_src[0] : '' ); ?>"
					/>

					<a href="#_action_icon" class="button button-small upload-icon upload-icon-image" data-uploader-button-text="<?php esc_attr_e( 'Set as status icon', 'open-custom-order-status' ); ?>"><?php esc_html_e( "Select File", 'open-custom-order-status' ); ?></a>
					<a href="#_action_icon" class="button button-small remove-icon" ><?php esc_html_e( "Remove Icon", 'open-custom-order-status' ); ?></a>
					<?php echo wc_help_tip( __( 'Optional action icon displayed in the action buttons for the next statuses.', 'open-custom-order-status' ) ); ?>
				</p>
			</div><!-- // .options_group -->
			...
			<div class="options_group">
				...
				// Next statuses
				$next_status_options = $this->nextStatusesOptions($status);
				$selected = $status->get_next_statuses();
				$selected = $selected ?: array();

				?>
				<p class="form-field _next_statuses_field">
					<label for="_next_statuses"><?php esc_html_e( 'Next Statuses', 'open-custom-order-status' ); ?></label>
					<select id="_next_statuses"
					        name="_next_statuses[]"
					        class="select short"
					        data-placeholder="<?php esc_html_e( 'Choose statuses that follow this one in your workflow.', 'open-custom-order-status' ); ?>"
					        multiple >
					...
					<?php echo wc_help_tip( __( 'Zero or more statuses that would be considered next during normal order status flow. Action buttons will be available to move an order with this custom status to these next statuses.', 'open-custom-order-status' ) ); ?>
				</p>
				<?php
				...
				woocommerce_wp_checkbox( array(
					'id'          => '_bulk_action',
					'label'       => __( 'Bulk action', 'open-custom-order-status' ),
					'description' => __( 'Check this to add this order status to the Orders list table Bulk Actions list.', 'open-custom-order-status' ),
				...
				woocommerce_wp_checkbox( array(
					...
					'label'       => __( 'Include in reports', 'open-custom-order-status' ),
					'description' => __( 'Check this to include orders with this order status in the order reports.', 'open-custom-order-status' ),
				...
				$is_paid_description       = __( 'Choose whether this status implies a payment that has been received or not.', 'open-custom-order-status' );
				...
				if (in_array($status->get_slug(), open_custom_order_status()->get_order_statuses_instance()->getCoreOrderStatusesThatRequirePayment(), true)) {
					...
					$is_paid_description  = '<span style="display: block; float: left; clear: both;"> ' . sprintf(
						...
						__( 'The Paid setting for %s cannot be changed to avoid order processing issues.', 'open-custom-order-status' ), $status->get_name()
					) . '</span>';
				...
				woocommerce_wp_select( array_merge( [
					'id'          => '_is_paid',
					'label'       => _x( 'Paid', 'Order status has had payment received', 'open-custom-order-status' ),
					'description' => $is_paid_description,
					'desc_tip'    => $is_paid_desc_tip,
					'value'       => $is_paid_value,
					'options'     => [
						'yes'           => __( 'Orders with this status have been paid.', 'open-custom-order-status' ),
						'needs_payment' => __( 'Orders with this status require payment (similar to "pending").', 'open-custom-order-status' ),
						'no'            => __( 'Orders are neither paid nor require payment (similar to "on-hold" or "refunded").', 'open-custom-order-status' ),
					],
				], $is_paid_custom_attributes ) );
			...
			<li class="wide">
				<div id="delete-action">
					<?php if ( ! $status->is_core_status() && current_user_can( "delete_post", $post->ID ) ) : ?>
						<a class="submitdelete deletion" href="<?php echo esc_url( get_delete_post_link( $post->ID, '', true ) ); ?>"><?php esc_html_e( 'Delete', 'open-custom-order-status' ); ?></a>
					<?php endif; ?>
				</div>
				...
				<input
					...
					value="<?php esc_attr_e( 'Save Order Status', 'open-custom-order-status' ); ?>"
					data-tip="<?php esc_attr_e( 'Save/update the order status', 'open-custom-order-status' ); ?>"
				/>
			</li>
			<?php
	...
	protected function nextStatusesOptions(Open_Custom_Order_Status_Order_Status $status) : array
	{
		$options = [];
		...
		foreach (wc_get_order_statuses() as $slug => $name) {
			...
			if ($status->get_slug(true) !== $slug) {
				$options[str_replace('wc-', '', $slug)] = $name;
			}
		}
	...
	public function save_order_status_meta( $post_id ) {
		...
		$status = new Open_Custom_Order_Status_Order_Status( $post_id );
		...
		if ($nextStatuses) {
			$validNextStatusOptions = array_keys($this->nextStatusesOptions($status));
			$nextStatuses = array_intersect($nextStatuses, $validNextStatusOptions);
		}

		update_post_meta($post_id, '_next_statuses', $nextStatuses);
		...
		update_post_meta($post_id, '_icon', sanitize_text_field($_POST['_icon'] ?? ''));
		update_post_meta($post_id, '_action_icon', sanitize_text_field($_POST['_action_icon'] ?? ''));

		$is_paid = sanitize_key($_POST['_is_paid'] ?? 'no');
		if (in_array($status->get_slug(), open_custom_order_status()->get_order_statuses_instance()->getCoreOrderStatusesThatRequirePayment(), true)) {
			$is_paid = 'needs_payment';
		}

		if (in_array($is_paid, ['needs_payment', 'yes', 'no'], true)) {
	...
	public function order_status_table_actions( $actions ) {
		...
		return array(
			'import-custom-statuses' => ' <button id="import-custom-statuses" class="button help_tip" data-tip="' . esc_attr__( 'Add statuses that may have been added by other plugins or custom code.', 'open-custom-order-status' ) . '" >' .
			                            esc_html__( 'Import custom statuses', 'open-custom-order-status' ) .
			                            '</button> ',
		);
	}
	...
	public function custom_column_content( $column, $post_id ) {
		...
		switch ( $column ) {
			...
			case 'icon';
				...
				if ( $color ) {
					...
					if ( $icon ) {
						...
						$style = 'background-color: ' . $color . '; color: ' . open_custom_order_status()->get_icons_instance()->get_contrast_text_color( $color ) . ';';
					}
				}
			...
			case 'reports' :
				echo esc_html( $status->include_in_reports() ? __( 'Yes', 'open-custom-order-status' ) : __( 'No', 'open-custom-order-status' ) );
			break;

			case 'paid':
				echo esc_html ( $status->is_paid() ? __( 'Yes', 'open-custom-order-status' ) : __( 'No', 'open-custom-order-status' ) );
			break;
	...
	public function reassign_order_status_popup() {
		...
		if ( 'wc_order_status' === $typenow ) :
			...
			<div id="reassign-order-status-popup" style="display: none;">
				...
				<h3><?php esc_html_e( 'Are you sure that you want to delete this order status?', 'open-custom-order-status' ); ?></h3>
				...
				<p class="singular" style="display: none">
					<a class="order-status-link"  href=""><?php
						...
						printf( esc_html__( 'There is currently %1$s order marked as %2$s.', 'open-custom-order-status' ), '<strong class="order-status-count"></strong>', '<strong class="order-status-name"></strong>' ); ?></a>
				</p>
				<p class="plural" style="display: none">
					<a class="order-status-link" href=""><?php
						...
						printf( esc_html__( 'There are currently %1$s orders marked as %2$s.', 'open-custom-order-status' ), '<strong class="order-status-count"></strong>', '<strong class="order-status-name"></strong>' ); ?></a>
				</p>

				<p><?php esc_html_e( 'You can choose to reassign the status of existing orders with another before deleting or have it reassigned automatically.', 'open-custom-order-status' ) ?></p>
				...
				<label for="wc-order-status-manager-reassign-status">
					...
					<button class="button reassign"><?php esc_html_e( 'Reassign and delete', 'open-custom-order-status' ); ?></button>
				</label>

				<button class="button delete"><?php esc_html_e( 'Delete', 'open-custom-order-status' ); ?></button>

				<p>
					<em><?php esc_html_e( 'Either operation cannot be undone automatically.', 'open-custom-order-status' ); ?></em><br>
					<em><?php esc_html_e( 'Emails and other actions will not be triggered automatically.', 'open-custom-order-status' ); ?></em>
				</p>
