 ...
 * @since 1.0.0
 */
class Open_Custom_Order_Status_Admin {


	/** @var Framework\SV_WP_Admin_Message_Handler instance **/
	private $message_handler;

	/** @var \Open_Custom_Order_Status_Admin_Orders instance **/
	private $admin_orders;

	/** @var \Open_Custom_Order_Status_Admin_Order_Statuses|\Open_Custom_Order_Status_Admin_Order_Status_Emails instance **/
	...
	public function __construct() {
		...
		$this->message_handler = open_custom_order_status()->get_message_handler();
	...
	public function print_settings_tabs() {
		...
		?>
		<a href="<?php echo esc_url( admin_url( 'edit.php?post_type=wc_order_status' ) ); ?>" class="nav-tab <?php echo ( 'wc_order_status' === $typenow ? 'nav-tab-active' : '' ); ?>"><?php esc_html_e( 'Order Statuses', 'open-custom-order-status' ); ?></a>
		<?php
	}
	...
	/**
	 * Output Open Custom Order Status sections
	 *
	...
	public function output_sections() {
		...
		?>
		<ul class="subsubsub">
			<li><a href="<?php echo esc_url( admin_url( 'edit.php?post_type=wc_order_status' ) ); ?>" class="<?php echo ( 'wc_order_status' === $typenow ? 'current' : '' ); ?>"><?php esc_html_e( 'Statuses', 'open-custom-order-status' ); ?></a> | </li>
			<li><a href="<?php echo esc_url( admin_url( 'edit.php?post_type=wc_order_email' ) ); ?>" class="<?php echo ( 'wc_order_email' === $typenow ? 'current' : '' ); ?>"><?php esc_html_e( 'Emails', 'open-custom-order-status' ); ?></a></li>
		</ul>
		<br class="clear" />
		<?php
	}
	...
	public function show_messages() {
		...
		open_custom_order_status()->get_message_handler()->show_messages();
	}
	...
	public function init() {
		...
		if ( $is_hpos_orders_screen || 'post-new.php' === $pagenow || 'post.php' === $pagenow || 'edit.php' === $pagenow ) {
			...
			if ( 'wc_order_status' === $typenow || ( isset( $_GET['post'] ) && 'wc_order_status' === get_post_type( wc_clean( $_GET['post'] ) ) ) ) {
				require_once( open_custom_order_status()->get_plugin_path() . '/src/admin/class-open-custom-order-status-admin-order-statuses.php' );
				$this->admin_order_statuses = new Open_Custom_Order_Status_Admin_Order_Statuses();
			}

			if ( 'wc_order_email' === $typenow || ( isset( $_GET['post'] ) && 'wc_order_email' === get_post_type( wc_clean( $_GET['post'] ) ) ) ) {
				require_once( open_custom_order_status()->get_plugin_path() . '/src/admin/class-open-custom-order-status-admin-order-status-emails.php' );
				$this->admin_order_statuses = new Open_Custom_Order_Status_Admin_Order_Status_Emails();
			}
			...
			if ( $is_hpos_orders_screen || ( 'shop_order' === $typenow || ( isset( $_GET['post'] ) && 'shop_order' === get_post_type( wc_clean( $_GET['post'] ) ) ) ) ) {
				require_once( open_custom_order_status()->get_plugin_path() . '/src/admin/class-open-custom-order-status-admin-orders.php' );
				$this->admin_orders = new Open_Custom_Order_Status_Admin_Orders();
			}
		}
	}
	...
	public function load_styles_scripts() {
		...
		$isScreen = $screen instanceof WP_Screen;

		// order status edit screen specific styles & scripts
		if ( $isScreen && 'wc_order_status' === $screen->id ) {
			...
			wp_enqueue_script( 'jquery-fonticonpicker', open_custom_order_status()->get_plugin_url() . '/assets/js/vendor/jquery.fonticonpicker.min.js', array( 'jquery' ), Open_Custom_Order_Status_Plugin::VERSION );

			wp_enqueue_style( 'ocs-jquery-fonticonpicker', open_custom_order_status()->get_plugin_url() . '/assets/css/admin/jquery-fonticonpicker.min.css', null, Open_Custom_Order_Status_Plugin::VERSION );

			wp_enqueue_media();
		}

		if ($isScreen && in_array($screen->id, ['edit-shop_order', 'woocommerce_page_wc-orders'], true)) {

			// Font Awesome font & classes
			wp_enqueue_style( 'font-awesome', open_custom_order_status()->get_plugin_url() . '/assets/css/font-awesome.min.css', null, Open_Custom_Order_Status_Plugin::VERSION );
		}
		...
		if ( ( $isScreen && 'plugins' === $screen->id ) || $this->is_order_status_manager_screen() ) {

			// load WC admin CSS
			wp_enqueue_style( 'woocommerce_admin_styles', WC()->plugin_url() . '/assets/css/admin.css' );

			// Admin CSS
			wp_enqueue_style( 'open-custom-order-status-admin', open_custom_order_status()->get_plugin_url() . '/assets/css/admin/open-custom-order-status-admin.min.css', array( 'woocommerce_admin_styles' ), Open_Custom_Order_Status_Plugin::VERSION );

			// WooCommerce font class declarations
			wp_enqueue_style( 'woocommerce-font-classes', open_custom_order_status()->get_plugin_url() . '/assets/css/woocommerce-font-classes.min.css', array( 'woocommerce_admin_styles' ), Open_Custom_Order_Status_Plugin::VERSION );

			// Font Awesome font & classes
			wp_enqueue_style( 'font-awesome', open_custom_order_status()->get_plugin_url() . '/assets/css/font-awesome.min.css', null, Open_Custom_Order_Status_Plugin::VERSION );

			// Magnific Popup CSS
			wp_enqueue_style( 'magnific-popup', open_custom_order_status()->get_plugin_url() . '/assets/css/vendor/magnific-popup.css' );

			// Magnific Popup JS
			wp_register_script( 'magnific-popup', open_custom_order_status()->get_plugin_url() . '/assets/js/vendor/jquery.magnific-popup.min.js', array( 'jquery' ), Open_Custom_Order_Status_Plugin::VERSION, true );

			// plugin admin JS
			wp_enqueue_script(
				'open-custom-order-status-admin',
				open_custom_order_status()->get_plugin_url() . '/assets/js/admin/open-custom-order-status-admin.min.js',
				array(
					'jquery',
					'jquery-tiptip',
					'jquery-ui-sortable',
				...
				\Open_Custom_Order_Status_Plugin::VERSION
			);
			...
			$script_data = array(
				...
				'i18n' => array(
					'remove_this_condition' => __( 'Remove this condition?', 'open-custom-order-status' ),
					'from_status'           => __( 'From Status', 'open-custom-order-status' ),
					'to_status'             => __( 'To Status', 'open-custom-order-status' ),
					'remove'                => __( 'Remove', 'open-custom-order-status' ),
					'any'                   => __( 'Any', 'open-custom-order-status' ),
					'remove_icon'           => __( 'Remove Icon', 'open-custom-order-status' ),
					'select_icon'           => __( 'Select Icon', 'open-custom-order-status' ),
					'all_icon_packages'     => __( 'All icon packages', 'open-custom-order-status' ),
					'search_icons'          => __( 'Search Icons', 'open-custom-order-status' ),
					'choose_file'           => __( 'Choose a file', 'open-custom-order-status' ),
					'close'                 => __( 'Close', 'open-custom-order-status' ),
					'confirm_any_status'    => __( 'Emails will not be sent for dispatch conditions where the To and From statuses are both \'Any\'. Continue anyway?', 'open-custom-order-status' ),
					'confirm_deactivate'    => __( 'Please be sure to remove custom statuses from orders before deactivating the plugin. Continue?', 'open-custom-order-status' ),
					'wc_prefix_disallowed'  => __( 'Sorry, slugs can\'t begin with "wc-".', 'open-custom-order-status' ),
				),
			);
			...
			if ( $isScreen && 'wc_order_status' === $screen->id ) {
				...
				$icon_options = array();
				foreach ( open_custom_order_status()->get_icons_instance()->get_icon_options() as $package => $icons ) {
			...
			wp_localize_script( 'open-custom-order-status-admin', 'wc_order_status_manager', $script_data );
		}
	}
	...
	public function normalize_columns( $columns ) {
		...
		$columns['title'] = __( 'Name', 'open-custom-order-status' );
	...
	public function normalize_row_actions( $actions, WP_Post $post ) {
		...
		if ( $this->is_order_status_manager_post_type( get_post_type( $post->ID ) ) ) {
			...
			$order_status = new Open_Custom_Order_Status_Order_Status( $post->ID );

			if ( current_user_can( 'delete_post', $post->ID ) && ! $order_status->is_core_status() ) {

				$actions['delete'] = sprintf(
					'<a class="submitdelete" title="%1$s" href="%2$s">%3$s</a>',
					esc_attr__( 'Delete this item permanently', 'open-custom-order-status' ),
					get_delete_post_link( $post->ID, '', true ),
					__( 'Delete Permanently', 'open-custom-order-status' )
				);
			}
		}
		...
		if ( $this->is_order_status_manager_post_type( $post->post_type ) ) {
			...
			if ( Framework\SV_WC_Helper::str_starts_with( $post->post_name, 'wc-' ) ) {
				...
				if ( is_numeric( $new_post_id ) ) {
					...
					open_custom_order_status()->get_admin_instance()->message_handler->add_warning( sprintf(
						/* translators: Placeholder: %s - custom order status slug */
						__( 'Sorry, slugs can\'t begin with "wc-". Order status has been renamed "%s".', 'open-custom-order-status' ),
						$post->post_name
					) );
				}
			}
	...
	/**
	 * Disable autosave for the Open Custom Order Status post types
	 *
	 * @since 1.0.0
	 */
